...
TRACE_BASE="/workspace/nvidia-resiliency-ext/experiments/straggler_injection/PP=2_experiment/traces"
...
for TYPE in ${INJECT_TYPES}; do
  if [ "${TYPE}" = "none" ]; then
    ...
    docker exec kevin_straggler_exp python3 \
      /workspace/nvidia-resiliency-ext/experiments/straggler_injection/PP=2_experiment/fr_straggler_analyzer.py \
